"""
Async rate limit manager for Gemini API based on usage tiers.
...
class AsyncRateLimitManager:
    """Async rate limit manager for Gemini API calls."""
    ...
    def __init__(self):
        ...
        self.state_file = Path(".cache") / "async_rate_limit_state.json"
        ...
        # Request tracking - model-specific
    ...
    def set_tier_and_model(self, tier: str, model: str):
        ...
        self._save_state()
        logger.info("Async rate limit manager set to %s tier with model %s", tier, model)
    ...
    def _load_rate_limits_config(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("Loaded async rate limits config from %s", self.config_file)
            return config
    ...
    def _get_fallback_config(self) -> dict[str, Any]:
        ...
        logger.warning("Using fallback async rate limits configuration")
        return {
            "models": {
                "gemini-2.5-flash": {
    ...
    def _load_state(self):
        ...
        try:
            ...
            logger.warning("Failed to load async rate limit state: %s", e)
    ...
    def _load_models_state_from_new_format(self, models_data: dict[str, Any], now: float, today: date) -> None:
        ...
        logger.debug("Loaded async model-specific rate limit state for %d models", len(self.model_states))
    ...
    def _load_state_from_legacy_format(self, data: dict[str, Any], now: float, today: date) -> None:
        ...
        self.model_states[model_name] = state
        logger.info("Migrated old format async rate limit state for model %s", model_name)
    ...
    def _save_state(self):
        ...
        try:
            ...
            logger.warning("Failed to save async rate limit state to %s: %s", self.state_file, e)
        except (TypeError, ValueError) as e:
            logger.warning("Failed to serialize async rate limit state: %s", e)
    ...
    async def wait_if_needed(self, estimated_tokens: int = 1000) -> bool:
        """
        Wait if necessary to respect rate limits for current model (async).
    ...
    async def get_status(self) -> dict:
        """Get current rate limit status for current model (async)."""
        async with self.request_lock:
            self._cleanup_old_records()
            limits = self.get_current_limits()
            state = self._get_model_state(self.current_model)
...
# Global instance
async_rate_limiter = AsyncRateLimitManager()
